-- HallPass — game achievements for a FRESH database.
--
-- The catalogue an admin provisions per game, and one row per (player,
-- achievement) carrying both progress and the unlock stamp.
--
-- `player_achievements` references `players(id)`, so `app/lib/players.sql` must
-- be applied first.
--
-- For an EXISTING database, run `scoreboard/migrations/009_achievements.sql`
...
--
-- WHY THESE TABLES EXIST AT ALL, given `app/lib/badges.ts` argues at length that
