  ...
  return (
    <div className="relative" ref={ref}>
      <button
        ...
        // The chip lives on `SiteHeader`'s bar, which fills with `--surface`, so
        // the resting state fills with `--surface-2`; `bg-surface` would have left
        // it invisible there. Its count is `text-zinc-600` rather than `--muted`
        // for the same reason the header's placeholder is: `--muted` on
        // `--surface-2` is 4.45:1 in the light theme, under AA for 14px text. The
        // lit state keeps `bg-accent-yellow` under `text-zinc-900` in EITHER
        // theme: a yellow chip is read against its own ink, not against the page.
        className={`inline-flex h-11 items-center gap-1.5 rounded-full px-3.5 text-sm font-black transition ${
          active
            ...
            : "bg-surface-2 text-zinc-600 dark:text-zinc-300 hover:text-foreground"
        }`}
      >
      ...
      {open && (
        <div className="absolute right-0 top-[calc(100%+8px)] z-50 w-64 rounded-2xl border border-border bg-surface p-4 shadow-2xl">
          ...
          <div className="mt-3 border-t border-border pt-2 text-[13px] font-bold text-foreground">
